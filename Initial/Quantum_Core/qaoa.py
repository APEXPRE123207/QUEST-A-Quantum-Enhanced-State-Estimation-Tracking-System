...
class QAOAOptimizer:
    ...
    def __init__(self, options: Dict[str, Any]):
        ...
        """
        self.reps = options.get('reps', 2)
        # Setup the core QAOA algorithm
        self.qaoa = QAOA(sampler=Sampler(), optimizer=COBYLA(), reps=self.reps)
    ...
    def decode_solution(self, solution: np.ndarray, waypoints: np.ndarray) -> np.ndarray:
        ...
        for t in range(num_timesteps):
            ...
            try:
                ...
                raise ValueError(f"Invalid solution vector: No waypoint chosen for timestep {t}")
