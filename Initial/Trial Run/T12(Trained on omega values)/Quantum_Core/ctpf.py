...
class QCTPF(CTPF):
    ...
    def predict(self, dt: float,
        ...
        for i in range(self.num_particles):
            ...
            # ---- Phase 6.3: quantum turn-rate correction ----
            # omega_q = (avg_values[0] - 0.5) * 0.6   # rad/s, scaled
            # self.particles[i, 6] += alpha_q * omega_q
            # # Clip
            ...
            if abs(self.particles[i, 6]) < 0.02:   # ~1 deg/s
                continue
            # ---- Phase 6.3: quantum turn-rate correction ONLY ----
            omega_q = (avg_values[0] - 0.5) * 0.6   # rad/s
            self.particles[i, 6] += alpha_q * omega_q
