...
class QCTPF(CTPF):
    ...
    def predict(self, dt: float,
        ...
        innovation = np.std(ranges)
        INNOV_THRESHOLD = 80.0
        ...
        if not self.params or self.transpiled_template is None:
            return

        # --- Build features ---
        feature_list = []
        for i in range(self.num_particles):
            f = self._build_features_for_particle(self.particles[i],
                                                ownship_position, i)
            feature_list.append(f)
        ...
        for feat in feature_list:
            if len(feat) < len(self.params):
                feat = np.pad(feat, (0, len(self.params) - len(feat)))
            pm = {p: v for p, v in zip(self.params, feat)}
            param_maps.append(pm)
        ...
        MAX_A = 20.0  # m/s^2 (conservative)

        # --- Apply small residual correction ---
        for i in range(self.num_particles):
            counts = result.get_counts(i)
            avg_values = np.zeros(self.qc_template.num_qubits)
            ...
            # Quantum residual acceleration
            a_q = (avg_values[0:3] - 0.5) * MAX_A
            a_norm = np.linalg.norm(a_q)
            if a_norm > MAX_A:
                a_q = a_q / (a_norm + 1e-9) * MAX_A
