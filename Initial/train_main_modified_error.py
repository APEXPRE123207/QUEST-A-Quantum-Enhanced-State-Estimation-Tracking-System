...
def evaluate_fitness(population: Population):
    ...
    for genome in population.population:
        ...
        try:
            ...
            last_accel = target.get_state()[2].copy() if 'last_accel' in locals() else np.zeros(3)
            ...
            for step in range(num_steps):
                ...
                # Acceleration Error (NEW)
                true_accel = target.get_state()[2]
                predicted_accel = predicted_output[0:3] # Assumes first 3 outputs map to acceleration
                accel_error = np.linalg.norm(predicted_accel - true_accel)
                
                # Combine errors (give acceleration more weight)
                total_error += (0.4 * velocity_error) + (0.6 * accel_error)
                
                last_velocity = target.velocity.copy()
                last_accel = true_accel.copy()
...
if __name__ == '__main__':
    ...
    CHECKPOINT_PATH = "checkpoints/qneat_checkpoint_400_gen.pkl"
